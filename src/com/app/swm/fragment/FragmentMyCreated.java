...
public class FragmentMyCreated extends Fragment implements IXListViewListener {
	...
	private void initView() {
		 ...
		 * 
		 * for (int i = 0; i < 10; i++) { group1.setAdmin("Johan"+i);
		 * group1.setPeopleNum(5); group1.setStartTime(new Date());
		 * group1.setPostTime(new Date()); groupList.add(group1); }
		 */
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// Group group = (Group) adapter.getItem(position - 2); //
				// 减去header
				// 还有xlistview的header
	...
	private void initEvent(View rootView) {
		...
		bn_refresh.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				/*
				 * loadNews(1, true); loadNotice(); LoginUtil.Login(context);
				 */
			}
		});
	}
	...
	public void onRefresh() {
		// loadNews(1, true);
		String curDate = TimeUtil.dateToString(new Date(),
				TimeUtil.FORMAT_MONTH_DAY_TIME_EN);
		SharedPreferencesUtil.add(context, "news_time", curDate);
		// 通知 不用刷新
		/*
		 * if (!notice_success) { loadNotice(); }
		 */
		onLoadMore();
		onLoad();
	}
	...
	private void onLoad() {
		...
		if (!"".equals(curDate)) {
			...
			curDate = TimeUtil.dateToString(new Date(),
					TimeUtil.FORMAT_MONTH_DAY_TIME_EN);
			mListView.setRefreshTime(curDate);
		}
